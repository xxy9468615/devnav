...
async function doSearch(query, limit) {
  ...
  return results.map((r, i) => ({
    rank: i + 1, id: r.id, title: r.title, url: r.url,
    description: (r.description || '').slice(0, 120),
    category: r.category, tags: r.tags || [],
    similarity: parseFloat(r.similarity), featured: r.featured,
  }));
}
...
app.post('/mcp/messages', async (req, res) => {
  ...
  try {
    ...
    switch (method) {
      case 'initialize':
        result = { protocolVersion: '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'devnav-search', version: '1.0.0' } };
        break;
      case 'tools/list':
        result = { tools: [{ name: 'search_resources', description: 'Semantic search devnav developer resources.', inputSchema: { type: 'object', properties: { query: { type: 'string', description: 'Search keywords, e.g. "免费图床", "React 部署", "AI API"' }, limit: { type: 'number', description: 'Results count (default 10, max 20)', default: 10 } }, required: ['query'] } } } };
        break;
      case 'tools/call': {
        ...
        if (name === 'search_resources') {
          ...
          result = { content: [{ type: 'text', text: JSON.stringify({ query, total: results.length, results }, null, 2) }] };
        } else {
...
app.post('/search', async (req, res) => {
  try {
    ...
    const results = await doSearch(query.trim(), Math.min(limit, 20));
