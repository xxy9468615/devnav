...
function extractTags(title, heading) {
  ...
  for (const [keyword, tag] of Object.entries(TAG_KEYWORDS)) {
    if (combined.includes(keyword) && !tags.includes(tag)) tags.push(tag);
  }
...
export function parseFreeForDev(markdown) {
  ...
  for (const line of lines) {
    ...
    if (line.trim() && !line.startsWith('#')) currentParent = null;
  }
  return resources;
}
...
export async function syncFreeForDev(supabase) {
  ...
  const { data: existingRows, error: fetchErr } = await supabase
    .from('resources').select('id, url').eq('source', 'free-for-dev');
  if (fetchErr) console.error('[free-for-dev] Error fetching existing rows:', fetchErr.message);
  ...
  for (let i = 0; i < resources.length; i += BATCH_SIZE) {
    const batch = resources.slice(i, i + BATCH_SIZE).map(r => ({
      ...r, updated_at: new Date().toISOString(), last_checked: new Date().toISOString(), is_alive: true,
    }));
    const { error } = await supabase.from('resources').upsert(batch, { onConflict: 'id' });
    if (error) console.error(`[free-for-dev] Upsert error (batch ${i}):`, error.message);
  ...
  if (removedUrls.length > 0) {
    for (let i = 0; i < removedUrls.length; i += BATCH_SIZE) {
      ...
      const { error } = await supabase.from('resources')
        ...
        .eq('source', 'free-for-dev').in('url', batch);
      if (error) console.error('[free-for-dev] Mark-remove error:', error.message);
